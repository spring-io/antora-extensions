  ...
  this.once('contentAggregated', async ({ playbook, contentAggregate }) => {
    for (const { origins } of contentAggregate) {
      for (const origin of origins) {
        const { url, gitdir, refhash, worktree, descriptor, tag, refname } = origin
        ...
        if (collectorConfig !== undefined && tag !== undefined) {
          ...
          if (!fs.existsSync(cacheDir)) {
            ...
            const content = await download(get, cacheUrl, logger).then((content) => content)
            if (content) {
          ...
          if (fs.existsSync(cacheDir)) {
            ...
            normalizedCollectorConfig.forEach((collector) => {
              ...
              normalizedScanConfigs.forEach((normalizedScanConfig) => {
                ...
                const cachedCollectorConfig = createCachedCollectorConfig(normalizedScanConfig.dir, cacheDir)
                logger.info(`Configuring collector to cache '${JSON.stringify(cachedCollectorConfig)}'`)
...
function createCachedCollectorConfig (scanDir, cacheDir) {
  return [
    {
      run: {
        command: `\${{env.NODE}} '${resolvedCopyRecursiveJs}' '.' '${cacheDir}'`,
        dir: scanDir,
      },
    },
  ]
