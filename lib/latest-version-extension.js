...
const TagSemVerRx = /^v?(\d+)\.(\d+)\.(\d+)(?:\.(\d+))?(?:-INTERNAL)?(?:-(RC|M)(\d+))?$/
  ...
  this.once('componentsRegistered', ({ contentCatalog }) => {
    ...
    for (const component of components) {
      for (const componentVersion of component.versions) {
        ...
        const versionSegment = typeof prerelease === 'string' && prerelease.endsWith('-SNAPSHOT') ? generation + prerelease : generation
...
function extractGeneration (version) {
  ...
  return parts.join('.') + (version && version.endsWith('-INTERNAL') ? '-INTERNAL' : '')
}
