...
const filterRefs = require('../node_modules/@antora/content-aggregator/lib/filter-refs.js')
...
module.exports.register = function ({ config = {} }) {
  ...
  this.once('playbookBuilt', async ({ playbook }) => {
    ...
    if (isAuthorMode && playbook.content.sources?.length !== 1) {
      console.log('Unable to use author mode with multiple sources, reverting to full build')
      return
    }
    const refPatternCache = Object.assign(new Map(), { braces: new Map() })
    const partialSources = []
    for (const source of playbook.content.sources) {
      const patterns = isBranch ? source.branches : source.tags
      if (isAuthorMode || filterRefs([refname], patterns, refPatternCache)?.length) {
        Object.assign(source, isBranch ? { branches: [refname], tags: [] } : { branches: [], tags: [refname] })
        if (isAuthorMode) {
          Object.assign(source, { url: resolveAuthorUrl(playbook.dir) })
        }
        partialSources.push(source)
      }
    }
    if (!partialSources.length) {
      console.log('Unable find any matching sources, reverting to full build')
      return
    }
    playbook.content.sources = partialSources
