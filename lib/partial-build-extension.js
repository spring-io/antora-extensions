...
module.exports.register = function ({ config = {} }) {
  ...
  if (!refname) return
  const logger = this.getLogger('partial-build-extension')
  this.once('playbookBuilt', async ({ playbook }) => {
    ...
    const siteManifestData = await (
      siteManifestUrl.startsWith('https://') || siteManifestUrl.startsWith('http://')
        ? download(get, siteManifestUrl, logger)
    ...
    if (!version) {
      ...
      version = await getMavenVersion(get, baseUrl, refname, logger).catch((me) =>
        getGradleVersion(get, baseUrl, refname, logger).catch((ge) => {
          throw new Error(
...
function download (get, url, logger) {
...
function getGradleVersion (get, baseUrl, refname, logger) {
  const gradlePropertiesUrl = `${baseUrl}/gradle.properties`
  return download(get, gradlePropertiesUrl, logger).then((contents) => extractGradleVersion(contents, refname))
}
...
function getMavenVersion (get, baseUrl, refname, logger) {
  const pomUrl = `${baseUrl}/pom.xml`
  return download(get, pomUrl, logger).then((contents) => extractMavenVersion(contents, refname))
}
