...
DEPRECATED: This extension is no longer applied by default by the <<_antora-extensions>> bundle.
Antora 3.2.0-alpha.9 and later already propagate roles onto navigation entries natively (see https://gitlab.com/antora/antora/-/issues/701), so upgrade Antora and remove this extension from your playbook instead of registering it explicitly.
