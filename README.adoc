...
templatePath (default is to use the default template)::
Allows overriding the default handlebars template used to generate the configuration.

indexName (default is the latest version's name + -docs)::
...
rootComponentName (default is to error on ROOT component name)::
